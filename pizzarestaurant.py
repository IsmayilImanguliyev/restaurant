    ...
    while True:
        try:
            ...
            if choice_index < 0 or choice_index >= len(choices):
                raise ValueError("Invalid choice.Select a valid option.")
...
def handle_payment(total):
    ...
    while payment < total:
        print("Available bills for payment are: 1, 5, 10, 20, 50, 100")
        try:
            bill = int(input("Enter the bill that you are going to use for this payment: "))
            if bill not in bills:
                raise ValueError("Invalid bill.Select from the available options.")
